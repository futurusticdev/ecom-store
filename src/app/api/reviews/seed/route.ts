...
// Fake reviews data with varying ratings and content
const fakeUsers = [
  {
    id: "fake-user-1",
    name: "John Doe",
...
function getReviewContentForRating(rating: number) {
...
function getReviewCommentForRating(rating: number) {
...
export async function GET(req: NextRequest) {
  try {
    ...
    // Generate a set of fake reviews
    const reviews = [];
    ...
    for (let i = 0; i < count; i++) {
      ...
      await fetch("/api/reviews", {
        ...
        headers: {
          ...
          // Add a special header to bypass authentication in the reviews POST handler
    ...
    return NextResponse.json({
      message: `Queued ${count} fake reviews for product ${productId}`,
      reviews,
    });
  } catch (error) {
